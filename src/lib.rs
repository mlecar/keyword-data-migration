...
pub fn save_migration_statistic<'a>(conn: &MysqlConnection,
                                    unused_count_param:&i64,
                                    migrated_from_id_param:&i64,
                                    migrated_to_id_param:&i64,
                                    migration_step_param:&str) -> Result<(), Error> {
    use schema::migration_statistics::dsl::*;

    let new_migration_statistic = (unused_count.eq(unused_count_param),
